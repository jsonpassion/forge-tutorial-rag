...
**선수 지식**:
- [RAG 실패 패턴 분류와 진단](18.1)에서 배운 RAG Triad과 실패 유형
- [검색 단계 디버깅과 최적화](18.2)의 LoggingRetriever 패턴
- [생성 단계 최적화](18.3)의 프롬프트 설계 패턴
- [지연시간과 비용 최적화](18.4)의 PipelineProfile 개념
...
**3단계: 단계 격리** — [RAG 실패 패턴 분류와 진단](18.1)에서 배운 것처럼, 검색이 문제인지 생성이 문제인지를 먼저 구분합니다. 트레이스에서 리트리버의 출력(검색된 문서)을 확인하면 바로 알 수 있죠.

**4단계: 원인 수정** — 검색 문제라면 [검색 단계 디버깅과 최적화](18.2)의 파라미터 그리드 서치를, 생성 문제라면 [생성 단계 최적화](18.3)의 프롬프트 엔지니어링을 적용합니다.
...
프로덕션 환경에서는 개별 요청의 메트릭을 **집계**하여 대시보드에 표시해야 합니다. [지연시간과 비용 최적화](18.4)에서 배운 프로파일링을 관찰 체계에 통합하는 방법입니다:
...
다음 Chapter 19 [멀티모달 RAG — 이미지와 테이블 처리](ch19)에서는 텍스트를 넘어 **이미지와 테이블**까지 처리하는 RAG 시스템을 구축합니다. 이번 장에서 배운 관찰 도구들은 멀티모달 RAG의 복잡한 파이프라인을 디버깅할 때도 그대로 활용할 수 있습니다.
