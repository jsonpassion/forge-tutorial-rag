...
**선수 지식**:
- [17.1 RAG 평가란](./17-1.md)에서 배운 `SingleTurnSample`과 `EvaluationDataset`의 4가지 필드 구조
- [17.2 Faithfulness와 Answer Relevancy](./17-2.md), [17.3 Context Precision과 Recall](./17-3.md)에서 배운 핵심 메트릭의 작동 원리
...
[17.1](./17-1.md)에서 배운 `SingleTurnSample`의 4가지 필드를 떠올려보세요:
...
지금까지 개별 메트릭(17.2~17.3)과 데이터셋(17.4)을 따로따로 배웠습니다. 다음 세션 **[17.5 종합 평가 파이프라인과 자동화](./17-5.md)**에서는 이 모든 조각을 하나로 합칩니다. RAGAS의 `evaluate()` 함수로 모든 메트릭을 한 번에 측정하고, CI/CD에 통합하여 RAG 파이프라인 업데이트 때마다 자동으로 성능 리포트를 생성하는 방법을 배웁니다.
