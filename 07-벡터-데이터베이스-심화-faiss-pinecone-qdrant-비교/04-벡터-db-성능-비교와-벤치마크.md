...
한편, [7.2](07-벡터-데이터베이스-심화-faiss-pinecone-qdrant-비교/02-pinecone-관리형-벡터-데이터베이스.md)에서 배운 **Pinecone**은 이번 벤치마크 대상에 포함하지 않았습니다. Pinecone은 완전 관리형(managed) 클라우드 서비스이기 때문에, 로컬 벤치마크 환경에서 측정하면 **네트워크 지연(latency)**이 결과에 포함되어 나머지 셋과 공정한 비교가 어렵기 때문입니다. Pinecone의 공식 벤치마크 수치와 선택 기준은 [7.5 선택 가이드](07-벡터-데이터베이스-심화-faiss-pinecone-qdrant-비교/05-벡터-db-선택-가이드-의사결정-프레임워크.md)에서 함께 다룹니다.

**선수 지식**: [6장 ChromaDB](06-벡터-데이터베이스-기초-chromadb로-시작하기/01-벡터-데이터베이스란-왜-필요한가.md)의 기본 CRUD 연산, [7.1 FAISS](07-벡터-데이터베이스-심화-faiss-pinecone-qdrant-비교/01-faiss-대규모-벡터-검색의-표준.md)의 인덱스 타입(IndexFlatL2, IndexIVFFlat, IndexHNSWFlat), [7.2 Pinecone](07-벡터-데이터베이스-심화-faiss-pinecone-qdrant-비교/02-pinecone-관리형-벡터-데이터베이스.md)의 관리형 서비스 개념, [7.3 Qdrant](07-벡터-데이터베이스-심화-faiss-pinecone-qdrant-비교/03-qdrant-고성능-하이브리드-검색-엔진.md)의 컬렉션/포인트/페이로드 구조
