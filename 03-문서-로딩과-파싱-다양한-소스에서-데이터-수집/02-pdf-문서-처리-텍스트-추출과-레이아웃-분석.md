...
앞서 [문서 로딩 기초 — LangChain Document Loaders](03-문서-로딩과-파싱-다양한-소스에서-데이터-수집/01-문서-로딩-기초-langchain-document-loaders.md)에서 `Document` 객체의 구조와 다양한 로더의 기본 사용법을 배웠습니다. 이번 섹션에서는 실무에서 가장 빈번하게 마주치는 **PDF 문서**에 집중합니다. PDF는 "보이는 대로" 렌더링하기 위해 설계된 포맷이라, 텍스트를 "읽히는 순서대로" 추출하는 것이 생각보다 훨씬 어렵습니다.
