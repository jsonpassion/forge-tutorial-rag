...
[Ch2 세션 5](ch02/session05)에서 RAG 아키텍처를 직접 설계해 보셨죠? 그때 인제스천 파이프라인의 첫 번째 컴포넌트로 **문서 로더(Document Loader)**를 선택하셨을 겁니다. 이제 그 설계를 실제 코드로 구현할 차례입니다.
...
**선수 지식**: [Ch2: RAG 아키텍처](ch02)에서 배운 RAG 파이프라인의 전체 구조(Indexing → Retrieval → Generation)를 알고 있어야 합니다. 특히 Indexing 단계에서 문서 로딩이 어떤 역할을 하는지 기억해 주세요.
...
지금까지 텍스트, CSV, JSON이라는 비교적 단순한 형식을 다뤘는데요. 실무에서 가장 많이 마주치는 문서 형식은 사실 **PDF**입니다. 다음 섹션 [3.2: PDF 문서 로딩과 파싱](ch03/session02)에서는 `PyPDFLoader`, `PDFMinerLoader` 등 다양한 PDF 로더를 비교하고, 표·이미지가 포함된 복잡한 PDF를 효과적으로 처리하는 전략을 알아봅니다.
