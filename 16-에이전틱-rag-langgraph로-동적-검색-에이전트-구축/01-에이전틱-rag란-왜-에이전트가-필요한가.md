...
**선수 지식**: 앞서 [챕터 8](ch08/session1.md)~[챕터 10](ch10/session1.md)에서 배운 기본 RAG 파이프라인(검색 → 생성) 구조, LangChain의 기본 체인 개념, 그리고 [챕터 12](ch12/session1.md)에서 다룬 리랭킹의 개념을 알고 있으면 좋습니다.
...
> ⚠️ **흔한 오해**: "CRAG는 그냥 리랭킹(Reranking)과 같은 거 아닌가요?" — 아닙니다! 리랭킹([챕터 12](ch12/session1.md)에서 배운)은 검색 결과의 **순서만 재조정**하는 반면, CRAG는 검색 결과의 **품질 자체를 판정**하고, 부족하면 **완전히 다른 소스(웹 검색)**로 전환합니다. 리랭킹은 "좋은 순서로 정렬"이고, CRAG는 "이게 쓸 만한가?"를 판단하는 거죠.
