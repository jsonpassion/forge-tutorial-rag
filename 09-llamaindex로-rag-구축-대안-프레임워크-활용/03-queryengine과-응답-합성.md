...
이 섹션에서는 LlamaIndex RAG 파이프라인의 **후반부**, 즉 검색된 노드를 어떻게 최종 답변으로 합성하는지를 깊이 있게 다룹니다. 앞서 [세션 9.2: VectorStoreIndex — 인덱싱과 검색](session-9.2)에서 `as_query_engine()`을 간단히 사용해봤는데요, 이번에는 그 내부를 열어보고 각 부품을 직접 조립해보겠습니다.

**선수 지식**: 
- [세션 9.1](session-9.1)의 Document, Node, Index 개념
- [세션 9.2](session-9.2)의 VectorStoreIndex, `as_query_engine()` 기본 사용법
...
앞서 [세션 9.2](session-9.2)에서 `index.as_query_engine(similarity_top_k=3)`처럼 한 줄로 사용했는데, 이것은 내부적으로 아래와 같은 과정을 수행합니다:
...
[챕터 8](chapter-8)에서 배운 LangChain의 체인과 비교해보면, 두 프레임워크의 설계 철학 차이가 명확하게 드러납니다:
...
이번 섹션에서는 단일 질문에 대한 답변을 생성하는 **QueryEngine**을 집중적으로 다뤘습니다. 하지만 실제 사용자는 한 번 질문하고 끝나지 않죠 — "좀 더 자세히 알려줘", "다른 관점에서는?"처럼 **대화를 이어가고** 싶어 합니다. 다음 섹션인 [세션 9.4: ChatEngine — 멀티턴 대화 RAG](session-9.4)에서는 대화 히스토리를 유지하면서 RAG를 수행하는 **ChatEngine**을 배우고, QueryEngine과의 차이점을 비교해보겠습니다.
