...
이 섹션에서는 벡터 검색에 **구조화된 메타데이터 필터**를 결합하여 검색 정밀도를 크게 향상시키는 방법을 학습합니다. 앞서 [10.1: 유사도 검색 심화](10-검색-품질-향상-유사도-검색과-메타데이터-필터링/01-유사도-검색-심화-top-k와-임계값-최적화.md)에서 top-k와 임계값으로 결과 수를 조절하고, [10.2: MMR](10-검색-품질-향상-유사도-검색과-메타데이터-필터링/02-mmr-관련성과-다양성의-균형.md)에서 다양성을 확보하는 방법을 배웠는데요. 이번에는 "어떤 문서를 검색 대상에 포함할 것인가"를 **메타데이터 조건**으로 제어하는 전략을 다룹니다. 특히 [10.2: MMR](10-검색-품질-향상-유사도-검색과-메타데이터-필터링/02-mmr-관련성과-다양성의-균형.md)의 다음 섹션 미리보기에서 예고한 **MMR과 메타데이터 필터링의 결합 전략**도 함께 살펴봅니다.
...
메타데이터 필터링을 적용하면 이런 문제를 원천적으로 차단할 수 있습니다. 그리고 [10.2: MMR](10-검색-품질-향상-유사도-검색과-메타데이터-필터링/02-mmr-관련성과-다양성의-균형.md)에서 배운 다양성 확보 전략과 결합하면, "올바른 범위 안에서 다양한 관점의 문서"를 검색할 수 있어 더욱 강력해지죠. 실제로 많은 프로덕션 RAG 시스템이 메타데이터 필터링을 기본 전략으로 채택하고 있거든요.
...
문서를 벡터 DB에 저장할 때, `page_content`(텍스트 내용) 외에 **metadata** 딕셔너리를 함께 저장할 수 있습니다. [Session 3.1](03-문서-로딩과-파싱-다양한-소스에서-데이터-수집/01-문서-로딩-기초-langchain-document-loaders.md)에서 `Document` 객체를 배웠을 때 이미 이 구조를 접했는데요, 이제 이 메타데이터를 **검색 필터**로 활용하는 것이 핵심입니다.
...
[10.2: MMR](10-검색-품질-향상-유사도-검색과-메타데이터-필터링/02-mmr-관련성과-다양성의-균형.md)에서 배운 MMR은 검색 결과의 **다양성**을 확보하는 전략이었죠. 메타데이터 필터링은 검색 **범위**를 제한하는 전략이고요. 이 둘을 결합하면 "올바른 범위 안에서 다양한 관점의 문서를 검색"할 수 있어 프로덕션 RAG 시스템에서 매우 흔하게 사용되는 패턴입니다.
...
> 💡 **알고 계셨나요?**: [10.2: MMR](10-검색-품질-향상-유사도-검색과-메타데이터-필터링/02-mmr-관련성과-다양성의-균형.md)에서 배운 MMR과 메타데이터 필터링은 서로 보완적입니다. 메타데이터 필터로 **검색 범위**를 좁히고, MMR로 **결과의 다양성**을 확보하면 더욱 질 좋은 검색 결과를 얻을 수 있습니다. 필터링으로 후보 풀이 줄어들면 `fetch_k`도 그에 맞게 조절하는 것이 좋아요 — 필터 후 남는 문서 수보다 `fetch_k`가 크면 의미가 없으니까요.
