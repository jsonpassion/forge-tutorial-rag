...
**선수 지식**:
- [19.1 멀티모달 RAG 아키텍처](19-멀티모달-rag-이미지와-테이블-처리/01-멀티모달-rag-아키텍처-텍스트를-넘어서.md)에서 배운 세 가지 접근법(텍스트 변환, 멀티모달 임베딩, 비전 LLM)
- [19.2 테이블과 이미지 추출](19-멀티모달-rag-이미지와-테이블-처리/02-테이블과-이미지-추출-문서-파싱-심화.md)에서 배운 Unstructured.io의 `hi_res` 전략과 요소 분류
- [19.3 멀티모달 임베딩과 검색](19-멀티모달-rag-이미지와-테이블-처리/03-멀티모달-임베딩과-검색.md)에서 배운 CLIP과 Multi-Vector Retriever
...
이 파이프라인의 핵심 설계 원칙은 **"검색은 요약으로, 답변은 원본으로"**입니다. [19.3](19-멀티모달-rag-이미지와-테이블-처리/03-멀티모달-임베딩과-검색.md)에서 배운 Multi-Vector Retriever가 이 분리를 가능하게 하는데요, 요약 텍스트로 빠르게 관련 문서를 찾고, 실제 답변 생성에는 원본 텍스트·테이블·이미지를 LLM에 전달합니다.
...
[19.2](19-멀티모달-rag-이미지와-테이블-처리/02-테이블과-이미지-추출-문서-파싱-심화.md)에서 추출한 세 가지 요소를 통합 인덱싱하는 핵심 코드입니다.
...
[Ch17: RAG 평가](17-rag-평가-ragas-프레임워크로-시스템-성능-측정/01-rag-평가란-무엇을-어떻게-측정할-것인가.md)에서 배운 Faithfulness, Answer Relevancy 개념을 기억하시나요? RAGAS 프레임워크는 v0.2부터 이 메트릭들의 **멀티모달 확장 버전**을 제공합니다. `MultiModalFaithfulness`는 생성된 답변이 텍스트와 이미지 컨텍스트에 얼마나 충실한지를, `MultiModalRelevance`는 검색된 멀티모달 컨텍스트가 질문에 얼마나 관련 있는지를 측정합니다. 기존 텍스트 전용 메트릭과 동일한 원리이지만, 비전 LLM을 활용해 이미지 컨텍스트까지 검증 범위를 확장한 것이죠.
...
파이프라인을 만들었으면 품질을 측정해야겠죠. [Ch17: RAG 평가](17-rag-평가-ragas-프레임워크로-시스템-성능-측정/01-rag-평가란-무엇을-어떻게-측정할-것인가.md)에서 배운 Faithfulness와 Answer Relevancy의 멀티모달 확장 버전을 활용해 평가를 수행합니다.
...
실전에서 멀티모달 RAG를 운영할 때는 몇 가지 추가 고려사항이 있습니다. [Ch18: RAG 최적화와 디버깅](18-rag-최적화와-디버깅-성능-개선-전략/01-rag-실패-패턴-분류와-진단.md)에서 다룬 비동기 처리, 캐싱, 점진적 인덱싱 등의 최적화 전략을 멀티모달 환경에 맞게 확장 적용해야 하죠.
...
**지연시간 최적화** — 인덱싱 단계에서 요약 생성이 병목이 됩니다. [Ch18](18-rag-최적화와-디버깅-성능-개선-전략/01-rag-실패-패턴-분류와-진단.md)에서 다룬 비동기 처리 패턴을 적용하면 `asyncio`와 배치 처리를 결합해 인덱싱 시간을 3~5배 단축할 수 있습니다.
...
**정확도 최적화** — [Ch12: 리랭킹](12-리랭킹으로-검색-정확도-높이기-cohere-rerank-활용/01-리랭킹의-원리-왜-초기-검색으로는-부족한가.md)에서 배운 리랭킹을 멀티모달 검색에도 적용할 수 있습니다. 초기 검색 결과(k=20)를 Cohere Rerank로 재정렬하여 상위 4개만 LLM에 전달하면, 불필요한 컨텍스트가 줄어 답변 품질이 향상됩니다.
...
> 🔥 **실무 팁**: 멀티모달 RAG의 인덱싱 비용이 부담된다면, [Ch18](18-rag-최적화와-디버깅-성능-개선-전략/01-rag-실패-패턴-분류와-진단.md)에서 소개한 **점진적 인덱싱** 전략을 멀티모달에 적용해 보세요. 처음에는 텍스트만 인덱싱하고, 테이블과 이미지가 필요한 쿼리가 실패할 때만 해당 모달리티를 추가하는 방식입니다. 80%의 질문은 텍스트만으로 답할 수 있는 경우가 많거든요.
...
이것으로 Chapter 19 멀티모달 RAG를 마무리합니다! 텍스트를 넘어 이미지와 테이블까지 활용하는 RAG 시스템을 처음부터 끝까지 구축하고 평가하는 방법을 배웠습니다. 다음 [Chapter 20: 프로덕션 RAG 시스템](20-프로덕션-rag-시스템-배포-모니터링-확장/01-fastapi로-rag-api-서빙.md)에서는 지금까지 만든 RAG 시스템을 **실제 서비스로 배포**하는 방법을 다룹니다. Docker 컨테이너화, API 서빙, 모니터링 대시보드, 그리고 트래픽 증가에 대응하는 스케일링 전략까지 — 개발에서 운영으로의 전환을 실습합니다.
