...
**선수 지식**: [15.1 Contextual RAG 소개](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/01-contextual-rag-소개-청크에-맥락을-더하다.md)에서 배운 컨텍스추얼 검색의 원리, [15.2 컨텍스추얼 청크 생성](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/02-컨텍스추얼-청크-생성-파이프라인-구현.md)의 파이프라인 구현, [15.3 하이브리드 검색](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/03-컨텍스추얼-임베딩-bm25-하이브리드-검색.md)의 ContextualHybridRetriever 클래스
...
> 📦 **RAGAS 핵심 메트릭 — 간단 정리**
>
> 이 섹션에서는 RAGAS를 **실용 도구**로 먼저 활용합니다. 각 메트릭의 수학적 정의와 내부 동작 원리는 [Chapter 17: RAG 평가와 최적화](17-rag-평가-ragas-프레임워크로-시스템-성능-측정/01-rag-평가란-무엇을-어떻게-측정할-것인가.md)에서 본격적으로 다룹니다.
>
> | 메트릭 | 한 줄 정의 | 측정 방식 (간략) |
> |--------|-----------|-----------------|
...
> 🔗 RAGAS의 각 메트릭이 내부적으로 어떤 프롬프트를 사용하고, 점수를 어떻게 계산하는지는 [Chapter 17: RAG 평가와 최적화](17-rag-평가-ragas-프레임워크로-시스템-성능-측정/01-rag-평가란-무엇을-어떻게-측정할-것인가.md)에서 심도 있게 다룹니다. 지금은 "이런 메트릭이 있고, 이렇게 사용한다" 수준으로 이해하면 충분합니다.
...
실제로 기본 RAG와 Contextual RAG를 **검색 메트릭 + RAGAS**로 비교 평가하는 전체 파이프라인을 구축합니다. 이 실습에서는 [15.3](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/03-컨텍스추얼-임베딩-bm25-하이브리드-검색.md)에서 구현한 `ContextualHybridRetriever`를 활용합니다.
...
놀라운 건, 이 논문이 발표된 지 1년 만에 GitHub 스타 7,000개를 넘기고 RAG 평가의 사실상 표준이 되었다는 점입니다. 특히 **reference-free 평가**가 가능하다는 점이 실무에서 큰 인기를 끌었습니다. 정답 데이터 없이도 검색 품질과 답변 품질을 측정할 수 있으니까요. RAGAS의 설계 원리와 각 메트릭의 수학적 배경은 [Chapter 17: RAG 평가와 최적화](17-rag-평가-ragas-프레임워크로-시스템-성능-측정/01-rag-평가란-무엇을-어떻게-측정할-것인가.md)에서 깊이 있게 다룰 예정입니다.
...
| **RAGAS** | RAG 시스템의 검색 품질과 생성 품질을 LLM 기반으로 자동 측정하는 평가 프레임워크. [Ch17](17-rag-평가-ragas-프레임워크로-시스템-성능-측정/01-rag-평가란-무엇을-어떻게-측정할-것인가.md)에서 심화 |
...
Chapter 15의 모든 여정을 마쳤습니다! Contextual RAG의 원리부터 구현, 하이브리드 검색, 그리고 성능·비용 분석까지 다뤘습니다. 다음 [Chapter 16: 에이전틱 RAG](16-에이전틱-rag-langgraph로-동적-검색-에이전트-구축/01-에이전틱-rag란-왜-에이전트가-필요한가.md)에서는 한 단계 더 나아가, **LLM 에이전트가 검색 전략을 동적으로 결정**하는 Agentic RAG를 LangGraph로 구축합니다. "검색할지 말지", "어떤 소스를 검색할지", "결과가 충분한지" — 이 모든 판단을 에이전트가 스스로 수행하는, RAG의 가장 진보된 형태를 만나게 됩니다.
