...
**선수 지식**: 
- [Ch4: 텍스트 청킹 전략](04-텍스트-청킹-전략-문서-분할과-최적화/01-청킹의-중요성과-기본-원리.md)에서 배운 청킹의 기본 원리와 다양한 분할 방법
- [Ch5: 임베딩 모델 이해](05-임베딩-모델-이해-텍스트를-벡터로-변환/01-임베딩의-기본-개념-단어에서-문장까지.md)에서 배운 텍스트 임베딩의 원리
- [Ch11: 하이브리드 검색](11-하이브리드-검색-bm25-키워드-검색과-벡터-검색-결합/01-bm25-키워드-검색-전통적-정보-검색의-힘.md)에서 배운 BM25 키워드 검색과 벡터 검색의 결합
- [Ch12: 리랭킹](12-리랭킹으로-검색-정확도-높이기-cohere-rerank-활용/01-리랭킹의-원리-왜-초기-검색으로는-부족한가.md)에서 배운 Cross-Encoder 기반 검색 결과 재정렬
- [Ch14: 고급 청킹과 인덱싱](14-고급-청킹과-인덱싱-raptor-시멘틱-청킹-부모-자식-청킹/01-부모-자식-청킹-작게-검색하고-크게-반환하기.md)에서 배운 시멘틱 청킹, 부모-자식 청킹 등 고급 전략
...
이것이 바로 RAG 시스템의 가장 흔하면서도 치명적인 문제인 **맥락 손실**입니다. [Ch14: 고급 청킹과 인덱싱](14-고급-청킹과-인덱싱-raptor-시멘틱-청킹-부모-자식-청킹/01-부모-자식-청킹-작게-검색하고-크게-반환하기.md)에서 부모-자식 청킹, 시멘틱 청킹 같은 고급 전략을 배웠지만, 이 전략들도 청크 자체에 맥락 정보를 포함시키지는 않았습니다. Anthropic의 Contextual Retrieval은 이 문제에 대한 놀랍도록 단순하면서 효과적인 해법을 제시합니다 — **검색 실패율을 최대 67%까지 줄일 수 있거든요.**
...
- **Contextual Embeddings**: 컨텍스트 접두사가 추가된 청크를 임베딩하여 벡터 검색에 사용. [Ch5: 임베딩 모델 이해](05-임베딩-모델-이해-텍스트를-벡터로-변환/01-임베딩의-기본-개념-단어에서-문장까지.md)에서 배운 것처럼, 텍스트 임베딩은 의미적 유사성을 벡터 공간에서 표현하는데, 맥락이 풍부한 텍스트를 임베딩하면 당연히 검색 품질도 올라가겠죠?
- **Contextual BM25**: 같은 컨텍스추얼 청크로 BM25 키워드 인덱스를 구축하여 키워드 검색에 사용

[Ch11: 하이브리드 검색](11-하이브리드-검색-bm25-키워드-검색과-벡터-검색-결합/01-bm25-키워드-검색-전통적-정보-검색의-힘.md)에서 배운 것처럼, 벡터 검색과 BM25를 결합하면 각각의 약점을 보완할 수 있었죠? Contextual Retrieval은 이 하이브리드 검색의 **입력 자체를 개선**하여 두 검색 모두의 품질을 한 단계 끌어올립니다.
...
3. **+ Reranking** — [Ch12](12-리랭킹으로-검색-정확도-높이기-cohere-rerank-활용/01-리랭킹의-원리-왜-초기-검색으로는-부족한가.md)에서 배운 리랭킹을 결합하여 최종 정밀도 극대화 (67% 개선)
...
이번 섹션에서는 Contextual Retrieval의 **핵심 아이디어와 왜 효과적인지**를 이해했습니다. 다음 섹션 **[15.2: 컨텍스추얼 청크 생성 파이프라인 구현](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/02-컨텍스추얼-청크-생성-파이프라인-구현.md)**에서는 실제로 LLM을 활용하여 컨텍스추얼 청크를 대규모로 생성하는 파이프라인을 직접 구축합니다. 프롬프트 캐싱 설정, 병렬 처리, 토큰 사용량 추적까지 프로덕션 수준의 구현을 다룹니다.
