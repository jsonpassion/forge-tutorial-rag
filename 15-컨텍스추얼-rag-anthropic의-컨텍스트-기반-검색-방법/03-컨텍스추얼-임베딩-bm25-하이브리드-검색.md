...
이 섹션에서는 [Session 15.2: 컨텍스추얼 청크 생성 파이프라인 구현](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/02-컨텍스추얼-청크-생성-파이프라인-구현.md)에서 만든 컨텍스추얼 청크를 실제 검색에 활용하는 방법을 배웁니다. 벡터 검색(임베딩)과 키워드 검색(BM25)을 동시에 수행하고, Reciprocal Rank Fusion(RRF)으로 결과를 결합한 뒤, 리랭킹 모델로 최종 정밀도를 끌어올리는 전체 파이프라인을 구현합니다.

**선수 지식**:
- [Session 15.1](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/01-contextual-rag-소개-청크에-맥락을-더하다.md)에서 배운 Contextual RAG의 핵심 아이디어와 컨텍스트 접두사 개념
- [Session 15.2](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/02-컨텍스추얼-청크-생성-파이프라인-구현.md)에서 구현한 컨텍스추얼 청크 생성 파이프라인 (`situate_context`)
- [Ch6: 벡터 데이터베이스 기초](06-벡터-데이터베이스-기초-chromadb로-시작하기/01-벡터-데이터베이스란-왜-필요한가.md)에서 배운 ChromaDB 컬렉션 생성과 쿼리
- [Ch11: 하이브리드 검색](11-하이브리드-검색-bm25-키워드-검색과-벡터-검색-결합/01-bm25-키워드-검색-전통적-정보-검색의-힘.md)에서 다룬 하이브리드 검색의 기본 개념, BM25, 그리고 [Ch11.2](11-하이브리드-검색-bm25-키워드-검색과-벡터-검색-결합/02-하이브리드-검색-구현-두-세계의-장점-결합.md)의 RRF 알고리즘
...
- [Ch11.2](11-하이브리드-검색-bm25-키워드-검색과-벡터-검색-결합/02-하이브리드-검색-구현-두-세계의-장점-결합.md)에서 배운 RRF 알고리즘을 컨텍스추얼 검색에 적용할 수 있다
...
[Ch11.2: 하이브리드 검색의 결합 전략](11-하이브리드-검색-bm25-키워드-검색과-벡터-검색-결합/02-하이브리드-검색-구현-두-세계의-장점-결합.md)에서 RRF(Reciprocal Rank Fusion) 알고리즘의 원리와 $\frac{1}{k + rank}$ 공식을 자세히 배웠습니다. 여기서는 RRF의 재정의 대신, **컨텍스추얼 검색에서 RRF가 왜 특히 잘 동작하는지**에 집중해 보겠습니다.
...
| RRF 적용 | [Ch11.2](11-하이브리드-검색-bm25-키워드-검색과-벡터-검색-결합/02-하이브리드-검색-구현-두-세계의-장점-결합.md)의 RRF 알고리즘을 컨텍스추얼 벡터+BM25 결과에 적용 |
...
다음 [Session 15.4: Contextual RAG 비용 분석과 성능 평가](15-컨텍스추얼-rag-anthropic의-컨텍스트-기반-검색-방법/04-contextual-rag-성능-평가와-비용-분석.md)에서는 프롬프트 캐싱의 실제 비용 절감 효과를 계산하고, RAGAS 프레임워크로 Contextual RAG의 Faithfulness, Context Precision 등 핵심 메트릭을 측정하여 투자 대비 효과를 정량적으로 분석합니다.
