...
지금까지 우리는 Jupyter 노트북이나 스크립트에서 RAG 파이프라인을 실행해왔습니다. 하지만 실제 서비스에서는 웹 앱, 모바일 앱, 다른 시스템이 HTTP 요청으로 RAG를 호출할 수 있어야 하죠. 이 세션에서는 FastAPI를 사용해 RAG 파이프라인을 REST API로 감싸고, 비동기 처리와 스트리밍 응답까지 구현합니다. 나아가 [Ch16에서 배운 에이전틱 RAG](16-에이전틱-rag-langgraph로-동적-검색-에이전트-구축/01-에이전틱-rag란-왜-에이전트가-필요한가.md)처럼 LangGraph 기반의 복잡한 그래프를 서빙할 때의 스트리밍 패턴과 상태 관리 전략도 다룹니다.

**선수 지식**: [Ch18 Session 18.1: RAG 파이프라인 디버깅](18-rag-최적화와-디버깅-성능-개선-전략/01-rag-실패-패턴-분류와-진단.md)에서 다룬 파이프라인 구조와 디버깅 기법, [Ch8 Session 8.1: 기본 RAG 파이프라인 구축](08-기본-rag-파이프라인-구축-langchain으로-첫-rag-앱-만들기/01-langchain-v1-핵심-개념과-설정.md)에서 만든 LangChain 기반 RAG 체인, [Ch16: 에이전틱 RAG](16-에이전틱-rag-langgraph로-동적-검색-에이전트-구축/01-에이전틱-rag란-왜-에이전트가-필요한가.md)에서 배운 LangGraph 기반 에이전트 구조
**학습 목표**:
...
실제로 기업에서 AI 프로젝트가 PoC(개념 증명)에서 프로덕션으로 넘어가지 못하는 가장 흔한 이유 중 하나가 바로 **서빙(Serving)** 문제입니다. 모델은 잘 동작하는데, 그것을 안정적인 API로 제공하는 방법을 모르는 거죠. 특히 [Ch16에서 다룬 에이전틱 RAG](16-에이전틱-rag-langgraph로-동적-검색-에이전트-구축/01-에이전틱-rag란-왜-에이전트가-필요한가.md)처럼 LangGraph 그래프가 여러 도구를 호출하며 분기하는 복잡한 파이프라인은, 단순한 LCEL 체인보다 서빙 난이도가 훨씬 높습니다. FastAPI는 Python 생태계에서 가장 인기 있는 비동기 웹 프레임워크로, LLM 기반 애플리케이션 서빙에 특히 적합합니다. 비동기 처리, 자동 문서 생성, 타입 검증까지 — RAG API 서빙에 필요한 모든 것을 제공하거든요.
...
> 💡 **비유**: 앞서 만든 LCEL 체인이 "한 줄로 쭉 이어진 컨베이어 벨트"라면, [Ch16에서 배운 LangGraph 에이전틱 RAG](16-에이전틱-rag-langgraph로-동적-검색-에이전트-구축/01-에이전틱-rag란-왜-에이전트가-필요한가.md)는 "여러 갈래로 분기하는 물류 센터"에 가깝습니다. 물류 센터에서는 어떤 택배가 어디를 거쳐가는지 추적해야 하고, 동시에 여러 라인이 돌아가니 상태 관리가 핵심이죠.
...
API 서버가 준비되었으니, 이제 그 뒤에서 데이터를 관리하는 방법을 알아볼 차례입니다. 다음 세션 **[20.2: 인덱스 관리와 데이터 파이프라인](20-프로덕션-rag-시스템-배포-모니터링-확장/02-인덱스-관리와-데이터-파이프라인.md)**에서는 문서의 추가·수정·삭제를 자동으로 감지하는 증분 인덱싱(Incremental Indexing)을 구축합니다. 지금 만든 FastAPI 앱이 항상 최신 데이터를 기반으로 응답할 수 있도록, 벡터 인덱스를 효율적으로 유지·갱신하는 전략을 배우게 됩니다.
