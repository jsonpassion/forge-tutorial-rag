...
이 섹션에서는 RAG 시스템이 실제 운영 환경에서 마주하는 가장 현실적인 문제—**"데이터가 바뀌면 인덱스를 어떻게 업데이트하지?"**—를 다룹니다. 앞서 [세션 20.1: FastAPI로 RAG API 서빙](ch20/session_20_1.md)에서 구축한 API 서버는 정적인 인덱스를 기반으로 동작했는데요, 실제 프로덕션에서는 문서가 매일 추가되고, 수정되고, 삭제됩니다. 이 변화를 인덱스에 반영하지 않으면 RAG 시스템은 점점 "오래된 지식"을 내놓는 시스템이 되어버리죠.

**선수 지식**: 
- [세션 20.1](ch20/session_20_1.md)의 FastAPI 기반 RAG 서비스 구조
- [Ch6: 벡터 데이터베이스 기초](ch06/)에서 다룬 벡터 DB의 기본 CRUD 연산
- [Ch4: 텍스트 청킹 전략](ch04/)에서 다룬 청킹 기법
- [Ch19: 멀티모달 RAG](ch19/)에서 다룬 이미지 임베딩과 멀티모달 검색 구조
...
[Ch19: 멀티모달 RAG](ch19/)에서 이미지와 텍스트를 함께 검색하는 시스템을 구축했는데요, 이를 프로덕션에 배포하면 인덱스 관리가 한층 복잡해집니다. 텍스트 임베딩과 이미지 임베딩이 **서로 다른 모델, 다른 차원, 다른 스토리지 특성**을 가지기 때문이죠.
