...
이 섹션에서는 키워드 기반 검색의 대표 알고리즘인 BM25(Best Matching 25)를 학습합니다. 앞서 [Ch5: 임베딩 모델 이해](05-임베딩-모델-이해-텍스트를-벡터로-변환/01-임베딩의-기본-개념-단어에서-문장까지.md)와 [Ch10: 검색 품질 향상](10-검색-품질-향상-유사도-검색과-메타데이터-필터링/01-유사도-검색-심화-top-k와-임계값-최적화.md)에서 벡터 유사도 검색을 깊이 다뤘는데요, 이번에는 그 반대편에 있는 키워드 검색의 세계로 들어갑니다. BM25는 단순해 보이지만, 특정 상황에서는 벡터 검색을 압도하는 성능을 보여줍니다.
